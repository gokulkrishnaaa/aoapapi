...
export const handleTransactionSync = async (req, res) => {
  ...
  try {
    ...
    await mainqueue.add("syncingTransactions", data);
...
export const syncingTransactions = async (data) => {
  console.log("syncing transactions", data);
  try {
    // get all entrance payments
    const transactions =
      data.type === "aeee"
        ? await prisma.entrancePayments.findMany()
        : data.type === "jee"
        ? await prisma.jEEPayments.findMany()
        : [];
    ...
    console.log("syncing error: Credentails wrong");
  }
};
